...
(defun bm/comparator-date-string (Adate Bdate)
  ...
  (cond ((< (string-to-int (first Adate))
	    (string-to-int (first Bdate)))
	 nil)
	((>  (string-to-int (first Adate))
	     (string-to-int (first Bdate)))
	 t)
	((=  (string-to-int (first Adate))
	     (string-to-int (first Bdate)))
	 (cond ((< (string-to-int (second Adate))
		   (string-to-int (second Bdate)))
		nil)
	       ((>  (string-to-int (second Adate))
		    (string-to-int (second Bdate)))
		t)
	       ((=  (string-to-int (second Adate))
		    (string-to-int (second Bdate)))
		(cond ((< (string-to-int (third Adate))
			  (string-to-int (third Bdate)))
		       nil)
		      ((>=  (string-to-int (third Adate))
			    (string-to-int (third Bdate)))
		       t)))))))
